...
def apply_chart_theme(fig, height=420):
    fig.update_layout(
        ...
        title=dict(y=0.98, yanchor="top", font=dict(size=18, color="#111827")),
...
def build_monthly_mileage_chart(runs_df):
    monthly_miles = (
    ...
    fig = px.area(
        ...
        labels={"start_time": "Month", "distance_miles": "Miles"},
    )
    fig.update_traces(line_color="#2E7D32", fillcolor="rgba(46, 125, 50, 0.18)")
...
def build_accuracy_stability_chart(summary_df):
    ...
    fig = px.scatter(
        ...
        labels={
            ...
            "peak_weekly_mileage": "Peak weekly mileage",
        },
        color_discrete_map={
            "optimistic": "#EF6C00",
            "conservative": "#1E88E5",
...
def build_volatility_by_block_chart(summary_df):
    ...
    fig = px.bar(
        ...
        orientation="h",
        color="prediction_bias",
        title="Rolling Prediction Volatility by Marathon Block",
        labels={
...
def build_prediction_error_chart(summary_df):
    ...
    fig = px.bar(
        ...
        y="prediction_error",
        color="prediction_bias",
        title="Final Prediction Error by Block",
        labels={
            "block_name": "",
...
def build_prediction_drift_chart(summary_df):
    ...
    fig = px.bar(
        ...
        y="prediction_drift",
        color="prediction_bias",
        title="Prediction Drift During the Block",
        labels={
            "block_name": "",
...
def build_prediction_timeline_by_block_chart(block_daily_df):
    ...
    fig = px.line(
        ...
        y=PREDICTION_COLUMN,
        color="block_name",
        title="Marathon Prediction Curves Aligned by Block Start",
        labels={
            "days_from_start": "Days from block start",
...
def build_volatility_relationship_chart(block_daily_df, x_column, title, x_label):
    chart_df = block_daily_df.dropna(subset=[x_column, "rolling_std_15_Marathond_x"]).copy()
    fig = px.scatter(
        chart_df,
        x=x_column,
...
def build_stability_error_chart(summary_df):
    chart_df = summary_df.copy()
    chart_df["absolute_error"] = chart_df["prediction_error"].abs()
    fig = px.scatter(
        chart_df,
...
def build_block_weekly_mileage_chart(block_df):
    weekly_df = (
    ...
    fig = px.bar(
        ...
        labels={"date": "Week ending", "total_distance_miles": "Miles"},
    )
    fig.update_traces(marker_color="#2E7D32")
    return apply_chart_theme(fig, height=360)


def build_block_daily_volume_chart(block_df):
    fig = go.Figure()
    fig.add_trace(
        go.Bar(
            x=block_df["date"],
            y=block_df["total_distance_miles"],
            name="Daily miles",
            marker_color="#90CAF9",
            opacity=0.85,
            hovertemplate="%{x|%b %d}<br>%{y:.1f} mi<extra></extra>",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=block_df["date"],
            y=block_df["total_distance_miles"].rolling(7, min_periods=1).mean(),
            name="7-day average",
    ...
    fig.update_layout(
        title="Daily Mileage and 7-Day Trend",
        xaxis_title="Date",
        yaxis_title="Miles",
        hovermode="x unified",
    )
